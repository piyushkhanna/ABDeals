...
myApp.service("browseAdsService",function($http,$rootScope){
	this.getData = function(startingPos,subCat){
		...
		if($rootScope.fields.browseAdsDealFilter == 0)
		{
			...
			return $http.get($rootScope.baseServiceURL + "/getAds.php?sortorder=start_date&subcat=" + subCat + "&limit=" + startingPos +  "&dev_id=" + $rootScope.dev_id + "&user_id=" + $rootScope.user_id + "&searchtype=l" + "&ulat=" + $rootScope.lat + "&ulng=" + $rootScope.lng + "&cntrycode=" + $rootScope.countryCode + "&pkms=50");

		}
		else
		{
			return $http.get($rootScope.baseServiceURL + "/getAds.php?sortorder=start_date&subcat=" + subCat + "&limit=" + startingPos + "&dev_id=" + $rootScope.dev_id + "&user_id=" + $rootScope.user_id + "&searchtype=n" + "&ulat=" + $rootScope.lat + "&ulng=" + $rootScope.lng + "&cntrycode=" + $rootScope.countryCode + "&pkms=50");
