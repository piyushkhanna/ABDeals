...
.controller('AppCtrl', function($scope,$rootScope, $ionicModal,$timeout,$state,$ionicHistory,ngFB,promotionsService,$sce) {
  ...
  $scope.chooseLocation = function() {
    ...
    { 
      ...
      google.maps.event.addListener(googleSearchBox, 'place_changed', function() {
        $rootScope.isLocationNameLoading = true;
  ...
  function setLocationBox(lat,lng)
  {
     ...
     geocoder.geocode({'latLng': pos}, function(results, status) {
        ...
        if($rootScope.appUser.get("pushToken") == undefined || $rootScope.appUser.get("pushToken") == null)
        {
         ...
         $rootScope.appUser.set("savedLocation", $rootScope.savedLocation);
         $rootScope.appUser.set("lat",$rootScope.lat);
         $rootScope.appUser.set("lng", $rootScope.lng);
         $rootScope.appUser.set("countryCode", $rootScope.countryCode);
         $rootScope.appUser.set("country", $rootScope.country);
        ...
        $timeout(function() {
          ...
          promotionsService.getPromotions().success(function(result){
            if(result != undefined && result.promotionid != undefined)
            {
                ...
                $rootScope.promotionObject.heading = $sce.trustAsHtml(result.heading);
                $rootScope.promotionObject.promotionid = result.promotionid;
                result.message = result.message.replace("##deviceid##",btoa($rootScope.dev_id));
                result.message = result.message.replace("##promoid##",btoa(result.promotionid));
                
                $rootScope.promotionObject.message = $sce.trustAsHtml(result.message);
                $rootScope.promotionObject.otherbuttontext = result.otherbuttontext;
                $rootScope.promotionObject.promotion = result.promotion;
            ...
            $rootScope.isLocationNameLoading = false;
          });
...
myApp.service("promotionsService",function($http,$rootScope){
  this.getPromotions = function(){
    return $http.get($rootScope.baseServiceURL + "/checkpromo.php?&dev_id=" + $rootScope.dev_id + "&dev_model=" + $rootScope.dev_model + "&dev_platform=" + $rootScope.dev_platform + "&dev_version=" + $rootScope.dev_version + "&ulat=" + $rootScope.lat + "&ulng=" + $rootScope.lng + "&cntrycode=" + $rootScope.countryCode + "&cuntry=" + $rootScope.country + "&preslocn=" + $rootScope.savedLocation);
  }
  this.denyPromotion = function()
  {
    return $http.get($rootScope.baseServiceURL + "/checkpromo.php?&dev_id=" + $rootScope.dev_id + "&dev_model=" + $rootScope.dev_model + "&dev_platform=" + $rootScope.dev_platform + "&dev_version=" + $rootScope.dev_version + "&ulat=" + $rootScope.lat + "&ulng=" + $rootScope.lng + "&cntrycode=" + $rootScope.countryCode +  "&cuntry=" + $rootScope.country + "&preslocn=" + $rootScope.savedLocation + "&notinterested=1&promoid=" + $rootScope.promotionObject.promotionid);
  }
});
