...
myApp.service("myperfectsService",function($http,$rootScope){
	this.getData = function(startingPos){
		...
		if($rootScope.fields.myPerfectsDealFilter == 0)
		{
			...
			return $http.get($rootScope.baseServiceURL + "/getAds.php?sortorder=distanta&getperfect=p&limit=" + startingPos +  "&dev_id=" + $rootScope.dev_id + "&user_id=" + $rootScope.user_id + "&searchtype=l" + "&ulat=" + $rootScope.lat + "&ulng=" + $rootScope.lng + "&cntrycode=" + $rootScope.countryCode + "&pkms=50");
		}
		else
		{
			return $http.get($rootScope.baseServiceURL + "/getAds.php?sortorder=distanta&getperfect=p&limit=" + startingPos + "&dev_id=" + $rootScope.dev_id + "&user_id=" + $rootScope.user_id + "&searchtype=n" + "&ulat=" + $rootScope.lat + "&ulng=" + $rootScope.lng + "&cntrycode=" + $rootScope.countryCode + "&pkms=50");
